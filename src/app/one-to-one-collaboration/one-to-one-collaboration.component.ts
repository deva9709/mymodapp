...
@Component({
  selector: 'app-one-to-one-collaboration',
  templateUrl: './one-to-one-collaboration.component.html',
  styleUrls: ['./one-to-one-collaboration.component.css']
})
export class OneToOneCollaborationComponent implements OnInit {
  roomName: string;
  sessionTitle: string;
  activeRoom: any;
  screenTrack: any;
  ...
  isVideoCallStoped: boolean = false;
  ...
  get controls(): any {
    return { videoStatus: this.videoStatus, micStatus: this.micStatus, screenShareStatus: this.screenShareStatus }
  }
  ...
  ngOnInit() {
    ...
    $(".master-container > .main-content").addClass('one-to-one-video-screen');
  }
  ...
  InitializeSession() {
    this.modservice.getToken(this.loginUserName).subscribe(data => {
      ...
      if (this.roomName) {
        let connectOptions = { name: this.roomName, logLevel: 'debug', tracks: undefined, audio: true, video: true }; // video flag should be set based on business logic
        // if (previewTracks) {
  ...
  roomJoined(room) {
    ...
    room.on('trackRemoved', (track, participant) => {
      ...
      if (track.kind === 'video' && track.name !== "screenshare") {
        ...
        this.isVideoCallStoped = true;
      }
      if (track.kind === 'video' && track.name === "screenshare") {
        if (!this.isVideoCallStoped) {
          this.videoIndex--;
          let remoteMediaDiv = document.getElementById("remote-media-section");
          remoteMediaDiv.className = ""; //removed min-remote-media
          let remoteMediaDivChild = remoteMediaDiv.children[0];
          this.renderer.setStyle(remoteMediaDivChild, 'height', 'calc(96vh - 50px)');
  ...
  ngOnDestroy() {
    ...
    $(".master-container > .main-content").removeClass('one-to-one-video-screen');//css(styles);
